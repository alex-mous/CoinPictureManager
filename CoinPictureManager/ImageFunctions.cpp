...
int createThumbnail(fs::path image_dir, int thumbnail_height, int max_pics) {
	...
	for (auto &f : fs::directory_iterator(image_dir)) { //Read each image file into vector
		if ((f.path().extension() == ".JPG" || f.path().extension() == ".jpg") && (c < max_pics || max_pics < 1)) { //Only read JPG files and upto max_pics (but not if max_pics is less than 0)
 ...
 */
int createWebp(fs::path image_dir, int quality) {
	vector<int> params;
